...
StackingAction::StackingAction(EventAction* EA)
 : G4UserStackingAction(), fEventAction(EA),
   ...
   fPhotoAuger(-1),fComptAuger(-1),fPixeGamma(-1),fPixeAuger(-1), fEBrem(-1),
   fIDdefined(false)
{
...
//G4double E_photon = 0;
//std::ofstream outfile("ephoton.txt");
...
StackingAction::ClassifyNewTrack(const G4Track* aTrack)
{
  ...
  if(!fIDdefined) {
    ...
    fEBrem = G4PhysicsModelCatalog::GetIndex("eBrem");
  }
  ...
  */
  ...
  if (aTrack->GetDefinition() == G4Gamma::Gamma()) {
    ...
    if(idx == fPhotoGamma || idx == fComptGamma) {
     ...
     analysisManager->FillH1(48,energy);
