...
void Run::SetPrimaries(G4ParticleDefinition* particle, G4double energy,G4ParticleDefinition* particle2, G4double energy2)
{ 
  fParticle = particle;
  fEkin = energy;
...
void Run::EndOfRun()
{
  ...
  G4cout << "  computed theta0 (Highland formula) electron         = "
         << ComputeMscHighland(fParticle,fEkin)/mrad << " mrad" << G4endl;
...
G4double Run::ComputeMscHighland(G4ParticleDefinition* particle, G4double energy)
{
  //compute the width of the Gaussian central part of the MultipleScattering
  //projected angular distribution.
  //Eur. Phys. Jour. C15 (2000) page 166, formule 23.9
